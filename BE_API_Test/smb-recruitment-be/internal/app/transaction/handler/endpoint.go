...
func NewTransactionEndpoint(userSessionHelper domain.UserSessionHelper, transactionService service.ITransactionService) *TransactionEndpoint {
	return &TransactionEndpoint{userSessionHelper: userSessionHelper, transactionService: transactionService}
}
...
func (transactionEndpoint *TransactionEndpoint) HandleCreateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	if err := request.Bind(r); err != nil {
		_ = render.Render(w, r, &TransactionHandlerFailed{
	...
	resp, err := transactionEndpoint.transactionService.CreateTransaction(request, r.Context())
	if err != nil {
		_ = render.Render(w, r, &TransactionHandlerFailed{
			HttpCode: http.StatusInternalServerError,
			Message:  err.Error(),
